...
export function TOCItems({ref, className, ...props}: ComponentProps<"div">) {
  ...
  return (
    <>
      <TocThumb containerRef={containerRef} className="absolute top-(--fd-top) h-(--fd-height) w-px bg-fd-primary transition-all" />
      <div ref={mergeRefs(ref, containerRef)} className={cn("flex flex-col border-s border-fd-foreground/10", className)} {...props}>
        {items.map((item) => (

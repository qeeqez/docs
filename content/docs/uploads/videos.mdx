...
description: Automated video transcoding system producing multiple quality levels and adaptive streaming formats for efficient delivery on any device.
---
...
## Advanced Processing Features
  ...
  <Cards>
    ...
    />
    <Card
     title="Frame Rate Intelligence" description="Maintains native frame rates and optimizes for target platforms"
    />
    <Card
       title="HDR Processing" description="Supports HDR with BT.2020 color space and 10-bit encoding"
    />
    <Card
title="Adaptive Quality Ladders" description="Generates multiple renditions even from low-resolution sources"    />
</Cards>
...
const completeResponse = await fetch(`https://api.rixl.com/v1/videos/upload/complete/${uploadData.video_id}`, {
  method: 'POST',
  headers: {
    'X-API-Key': 'YOUR_PROJECT_API_KEY'
  }
...
complete_response = requests.post(
    f"https://api.rixl.com/v1/videos/upload/complete/{upload_data['video_id']}",
...
# 3. Complete upload
curl -X POST https://api.rixl.com/v1/videos/upload/complete/$VIDEO_ID \
  -H "X-API-Key: YOUR_PROJECT_API_KEY"
```
</Tab>
</Tabs>

...
`VideoProps` also inherits standard HTML video props such as `className`, `muted`, `autoPlay`, `loop`, `playsInline`, and DOM event handlers.
...
These hooks all target a specific player instance and require the same `playerId` you pass to `<Video playerId={...} />`.
...
declare function useMediaSettings(playerId: UUID): UseMediaSettingsResult;
...
declare function usePlayback(playerId: UUID): UsePlaybackResult;
...
declare function usePlayerProgress(playerId: UUID): UsePlayerProgressResult;
...
declare function usePlayerUI(playerId: UUID): UsePlayerUIResult;
...
declare function usePlayerTracks(playerId: UUID): UsePlayerTracksResult;
```
...
interface UseGlobalPlayerSettingsResult {
  ...
  pauseAll: () => void;
  pauseOthers: (playerId: UUID) => void;
}
...
`useProgressBar` is a lower-level hook for a raw `HTMLVideoElement`. For the public `Video` component, prefer `usePlayerProgress(playerId)` when building custom controls.

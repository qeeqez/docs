...
<Callout type="info">
  Need custom controls instead of the built-in player UI? Pass a stable `playerId` to `<Video theme="hideUI" />` and pair it with hooks like [`usePlayback`](/en/sdk/hooks/use-playback) and [`usePlayerProgress`](/en/sdk/hooks/use-player-progress).
</Callout>

...
<Callout type="info">
  Audio track availability varies by video. Educational and professional content often includes multiple language
  options and commentary tracks.
</Callout>
...
<Callout type="info">
  Chapter shortcuts are only active when the video has chapters enabled via the `chapters` prop.
</Callout>
...
<Callout type="info">
  **Case Insensitive**: Letter shortcuts work with both uppercase and lowercase (e.g., both `f` and `F` trigger
  fullscreen).
</Callout>

<Callout type="info">
  **Context Aware**: Keyboard shortcuts are disabled when typing in input fields, text areas, or content-editable
  elements to prevent conflicts.
</Callout>

<Callout type="warning">
  **Focus Required**: The video player area must have focus for keyboard shortcuts to work. Click on the video if
  shortcuts aren't responding.
</Callout>
...
In the default theme, player controls automatically hide after **3 seconds** of inactivity. This applies in both normal and fullscreen modes.

- **Mouse movement** reveals the controls
- **Cursor hides** along with the controls when they auto-hide
- Controls **stay visible** while the settings menu is open
- Controls are **always visible** in the `feed` theme

### Action Feedback

When toggling play/pause, the player shows an animated feedback icon at the center of the video with a blurred backdrop. This provides clear visual confirmation of the action.

### Thumbhash Placeholder

While the video is loading, a blurred thumbhash placeholder image is displayed. This prevents layout shift and provides a smooth loading experience. The placeholder is generated from the video's poster image.

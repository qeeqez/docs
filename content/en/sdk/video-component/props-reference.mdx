...
### `id` **(Optional in type, usually required)**

**Type:** `string` | **Default:** `undefined`

The media identifier from your RIXL dashboard. In normal standalone usage, you should pass an `id` so the `Video` component can load media for you.
...
**Type:** `UUID` | **Default:** auto-generated per mounted player

Identifies a specific player instance. Pass your own stable `playerId` when using player hooks like `usePlayback`, `usePlayerProgress`, `usePlayerUI`, `usePlayerTracks`, or `useMediaSettings`.

```tsx
const playerId = "11111111-1111-1111-1111-111111111111";

<Video id="D3OCjdLP60" playerId={playerId} theme="hideUI" />
```

<Callout type="info">
  Custom controls work by sharing the same stable `playerId` between `<Video />` and your hooks. See the [Hooks overview](/en/sdk/hooks) for the supported composition pattern.
</Callout>
...
<Callout type="warning">
  **Hover theme exception:** When `theme="hover"` is set, `autoPlay` is ignored. The video is paused by default and only plays when the user hovers over it. See the [Hover Preview](/en/sdk/examples/hover-preview) example for details.
</Callout>
...
`VideoProps` also supports standard HTML video props through inheritance, including `className`, `muted`, `autoPlay`, `loop`, `playsInline`, and event handlers.

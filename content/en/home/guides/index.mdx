...
<Tabs items={['Video Player', 'Image Gallery', 'Content Feed']}>
  <Tab value="Video Player">
    ...
    function VideoPlayer({videoId, playerId}) {
...
<Callout type="info">
  If you want custom playback UI, give the `Video` component a stable `playerId` and use the same `playerId` with the
  hook APIs. Reach for `theme="hideUI"` when you want to replace the built-in controls entirely.
</Callout>
...
<Callout type="tip" title="Need inspiration?">
  ...
  Documentation](/en/home/platform) for comprehensive features, or dive into the [API
  Reference](/en/api) for detailed technical documentation.
</Callout>

...
<Columns cols={3}>
  <Card title="Adaptive HLS Streaming" icon="wifi">
    Multiple quality levels that automatically adjust based on connection speed
  ...
  <Card title="Quality Tiers" icon="settings">
    Basic (720p), Shorts (1080p), Pro (4K) processing options for different use
    cases.
  </Card>
  ...
  <Card title="Smart Compression" icon="zap">
    Optimal encoding settings that balance file size and visual quality
    automatically.
  </Card>
</Columns>
...
<Tab value="Advanced Player">
  ...
  function AdvancedVideoPlayer({videoId, playerId}) {
    const {paused, ended} = usePlayback(playerId);
    const {progress, duration, currentTime} = usePlayerProgress(playerId);
...
<Tab value="Custom Controls">
  ```jsx
  import {Video, useMediaSettings, usePlayback, usePlayerProgress} from '@rixl/videosdk-react';
  import {useState} from 'react';

  function CustomVideoPlayer({videoId, playerId}) {
    const [showControls, setShowControls] = useState(true);
    const {paused, togglePlay} = usePlayback(playerId);
    const {muted, toggleMute} = useMediaSettings(playerId);
    const {progress, duration, currentTime, seekTo} = usePlayerProgress(playerId);
...
<Callout type="info">
  For hook-driven controls, pass the same stable `playerId` to both `<Video />` and your player hooks. `theme="hideUI"`
  is the supported way to render the SDK player without built-in controls.
</Callout>
...
<Columns cols={3}>
  <Card title="Video Streaming Platform" icon="play">
    Netflix-style streaming with adaptive quality, progress tracking, and global
    delivery.
  </Card>
...
<div className="mt-12">
  <Callout type="tip" title="Ready to stream?">
    Start with our [Video Upload Guide](/en/home/platform/uploads/videos-guide) to see HLS streaming in action, or jump into the
    [React Component Guide](/en/sdk/video-component) to add professional video players to your application. For custom
    workflows, explore our [API Documentation](/en/api).
  </Callout>
</div>

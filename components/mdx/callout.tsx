...
const Callout = ({ type, title, children }: CalloutProps) => {
    ...
    return (
        <div className={`rounded-2xl border p-4 my-4 transition-colors ${bgClass} ${borderClass} ${textClass}`}>
            <div className="flex items-start gap-3">
                <IconComponent className={`h-5 w-5 mt-0.5 flex-shrink-0 ${iconClass}`} />
                <div className="flex-1 min-w-0">
                    {title ? (
                        <div className="flex items-center gap-2">
                            <h4 className="font-semibold text-current m-0">{title}</h4>
                        </div>
                    ) : null}
                    <div className={`text-sm leading-relaxed ${title ? "mt-1" : ""} [&>p]:mb-2 [&>p:last-child]:mb-0`}>
                        {!title ? <span className="align-middle">{children}</span> : children}
                    </div>
                </div>
            </div>
        </div>

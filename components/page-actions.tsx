...
export function LLMCopyButton({
  ...
  return (
    <div className={cn('rounded-lg [&_svg]:size-4 cursor-pointer border text-sm ' +
      'flex items-center h-7')}>
      <button
        type={`button`}
        disabled={isLoading}
        onClick={onClick}
        className={cn('flex items-center rounded-l-lg gap-2 cursor-pointer h-full px-2 border-r text-fd-accent-foreground hover:bg-fd-muted-foreground/10')}
      >
...
const optionVariants = cva(cn(
  'group p-2 [&_svg]:size-4 rounded-lg inline-flex items-center gap-2 cursor-pointer',
  "hover:bg-fd-muted-foreground/5",
  "text-sm"
));
...
export function ViewOptions({markdownUrl, githubUrl, onClick}: Props) {
  ...
  const renderOptionContent = (option: {
    ...
    <>
      <span
        className="w-10 h-10 border text-fd-muted-foreground/75 group-hover:text-fd-accent-foreground/80 flex items-center justify-center rounded-lg">
        {option.icon}
      </span>
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger
        className="cursor-pointer text-fd-accent-foreground hover:bg-fd-muted-foreground/10 rounded-r-lg px-2 h-full">
        <ChevronDown className="w-3.5 h-3.5"/>
      </PopoverTrigger>
      <PopoverContent
        align="end"
        className="flex flex-col overflow-auto border mt-2 bg-fd-background"
      >
        {options.map((option) =>
          option.href ? (
            <a
